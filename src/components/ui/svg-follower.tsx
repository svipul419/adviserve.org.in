...
export function SVGFollower({
  width = "100%",
  height = "100%",
  colors = ["#7F1D1D", "#991B1B", "#ffffff", "#444444", "#1A1F2E"],
  removeDelay = 400,
  className = "",
}: SVGFollowerProps) {
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (isRecording) {
        // Assume bounding client dimensions for scale metrics instead of static fixed width fallback if % used
        const w = typeof width === "number" ? width : rect.width || window.innerWidth
        const h = typeof height === "number" ? height : rect.height || window.innerHeight
