...
import type { WhyChooseItem } from "../../lib/defaults";

function getImageForIndex(items: WhyChooseItem[], index: number): string {
  return items[index]?.imageUrl || FALLBACK_IMAGES[index % FALLBACK_IMAGES.length];
}
...
interface WhyAdviserveTabsProps {
  items: WhyChooseItem[];
}

export function WhyAdvisserveTabs({ items }: WhyAdviserveTabsProps) {
    ...
    <div className="w-full">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
            ...
            {items.map((item, index) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group relative flex items-start gap-4 py-6 md:py-8 text-left transition-all duration-500 border-t border-black/10 first:border-0",
                    isActive
                      ? "text-black"
                  ...
                  <div className="flex flex-col gap-2 flex-1">
                    ...
                    <AnimatePresence mode="wait">
                      {isActive && (
                        ...
                        >
                          <p className="text-gray-500 text-sm md:text-[15px] font-normal leading-relaxed max-w-sm pb-2">
                            {item.description}
                          </p>
                        </motion.div>
                      )}
        ...
        <div className="lg:col-span-7 flex flex-col justify-end h-full order-1 lg:order-2">
          ...
          >
            <div className="relative aspect-[4/5] md:aspect-[4/3] lg:aspect-[16/11] rounded-2xl md:rounded-3xl overflow-hidden bg-gray-100 border border-black/5">
              <AnimatePresence initial={false} custom={direction} mode="popLayout">
                ...
                >
                  <img
                    src={getImageForIndex(items, activeIndex)}
              ...
              <div className="absolute bottom-6 right-6 md:bottom-8 md:right-8 flex gap-2 md:gap-3 z-20">
                <button
                  ...
                  className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/80 backdrop-blur-md border border-black/10 flex items-center justify-center text-black hover:bg-ink-glass transition-all active:scale-90"
                  aria-label="Previous"
                >
                  <ChevronLeft size={20} />
                </button>
...
export default WhyAdvisserveTabs;
