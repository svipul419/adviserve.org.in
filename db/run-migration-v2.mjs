/**
 * Run the V2 migration SQL against Neon database — statement by statement.
...
const migrationSql = readFileSync('db/migration-v2.sql', 'utf-8');
...
for (let i = 0; i < statements.length; i++) {
  ...
  try {
    ...
    if (stmt.startsWith('CREATE TABLE')) {
      ...
      const indexName = stmt.match(/CREATE\s+(?:UNIQUE\s+)?INDEX[^(]*?(\w+)\s+ON/)?.[1] || '?';
